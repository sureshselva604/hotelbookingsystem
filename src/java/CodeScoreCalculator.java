...
public class CodeScoreCalculator {
    ...
    public static String replaceQuestionIdsWithValues(Map<String, Double> scoresMap, String formula) {
        ...
        boolean containsNestedExpressions = true;

        // Loop until no nested expressions remain in the formula
        while (containsNestedExpressions) {
            ...
            Matcher matcher = pattern.matcher(replacedFormula);
            ...
            while (matcher.find()) {
                ...
                if (operation != null) {
                    // Recursive call to replace nested expressions inside the operation
                    String nestedExpression = replaceQuestionIdsWithValues(scoresMap, matcher.group(2));
                    tokens = nestedExpression.split(",");
            ...
            replacedFormula = sb.toString();

            // Check if the replaced formula still contains nested expressions
            containsNestedExpressions = replacedFormula.matches(".*\\([0-9.,\\s]+\\).*");
        }
    ...
    private static double calculateSum(Map<String, Double> scoresMap, String[] questionIds) {
        ...
        for (String id : questionIds) {
        	System.out.println("scoresMap:"+scoresMap.getOrDefault(id.trim(), 0.0));
